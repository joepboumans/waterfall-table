...
logger = logging.getLogger('FCM Test')
...
class FCMTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        logger.info("[INFO-FCM] Sending %d flows on port %d", NUM_FLOWS, ig_port)
        for i in range(NUM_FLOWS):
            ...
            if (i % 100 == 0):
                logger.info("[INFO-FCM] %d / %d flows are done...", i, NUM_FLOWS)

        
        logger.info("[INFO-FCM] Wait until packets are all received...")
        # check all packets are sent
        while True:
            register_pktcount = bfrt_info.table_get("FcmEgress.num_pkt")
            ...
            logger.info("[INFO-FCM] Sent : %d, Received : %d\t\t wait 10s more...", num_packets, data_dict["FcmEgress.num_pkt.f1"][0])
            if (data_dict["FcmEgress.num_pkt.f1"][0] == num_packets):
                break
            time.sleep(10)

        assert data_dict["FcmEgress.num_pkt.f1"][0] == num_packets, "Error: Packets are not correctly inserted..."
        ...
        for i in range(NUM_FLOWS):
            ...
            register_l1_d1 = bfrt_info.table_get("FcmEgress.fcmsketch.sketch_reg_l1_d1")
            ...
            if (val_d1 == ADD_LEVEL1):
                register_l2_d1 = bfrt_info.table_get("FcmEgress.fcmsketch.sketch_reg_l2_d1")
                ...
                if (val_d1 == ADD_LEVEL2):
                    register_l3_d1 = bfrt_info.table_get("FcmEgress.fcmsketch.sketch_reg_l3_d1")
            ...
            register_l1_d2 = bfrt_info.table_get("FcmEgress.fcmsketch.sketch_reg_l1_d2")
            ...
            if (val_d2 == ADD_LEVEL1):
                register_l2_d2 = bfrt_info.table_get("FcmEgress.fcmsketch.sketch_reg_l2_d2")
                ...
                if (val_d2 == ADD_LEVEL2):
                    register_l3_d2 = bfrt_info.table_get("FcmEgress.fcmsketch.sketch_reg_l3_d2")
